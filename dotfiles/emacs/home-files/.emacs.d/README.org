...
=pewkey= supports repeat mode.  Use =C-p= to enable while the keymap is activated and use =C-g= to exit.  I don't use the new =repeat-mode= because it doesn't work well with my workflow.
...
|-----------+--------------------+--------------------------+--------------------+-----------------------------------------------|
| =C-x=     | =pewkey=           | Any                      | My major keymap    | [[./pew/lisp/init-config.el][init-config.el]] |
| =C-x C-x= | =ctl-x-map=        | Any                      | Original C-x       | [[./pew/lisp/init-config.el][init-config.el]] |
| =C-z=     | =pewkey-prime-map= | Any                      | Random assignments | [[./pew/lisp/init-config.el][init-config.el]] |
...
* Debugging
Since I use Nix to manage all configurations including Emacs, debugging temporary changes in this repo could tedious. In that case, use [[./debug.sh][debug.sh]] to start an instance from this directory.
