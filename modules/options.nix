{
  ...
  userSettings = {
    ...
    options = {
      ...
      password = lib.mkOption {
        ...
        default = null;
        description = ''
          This option serves three purposes (from the highest precedence).
            - If the value is a path, it is taken as `hashedPasswordFile'
              regardless the `pix.system.immutableUsers' option.
            - If either of `pix.users.<name>.nologin' or `pix.system.immutableUsers'
              option is enabled, it is taken as `hashedPassword'.
            - Otherwise, set `initialPassword'.
    ...
    config = lib.mkIf config.nologin {
      password = lib.mkDefault "**DISABLED!**";
    };
  };
  ...
  definePassword = nologin: immutable: password:
    if lib.isPath password
    then {hashedPasswordFile = password;}
    else if nologin || immutable
    then {hashedPassword = password;}
    else {initialPassword = password;};
